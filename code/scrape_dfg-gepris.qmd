...
```{r}
f.scrape.gepris.project.desc <- function(project.id) {
...
f.scrape.gepris.project.collaborators <- function(project.id) {
  ...
  dplyr::tibble(
    ...
    collaborator.id = str_replace(
      f.get.text(v.page, paste(v.xpath.description, "div[@id = 'projekttext']/following-sibling::div[3]/span[@class = 'value']/a[starts-with(@href, '/gepris/person')]/@href", sep = "//")),
      '^.*?(\\d+)$', '\\1'),
    collaborator.name = f.get.text(v.page, paste(v.xpath.description, "div[@id = 'projekttext']/following-sibling::div[3]/span[@class = 'value']/a", sep = "/"))
  )
}
```

Test this scraping function with a complex project

```{r test-scrape-gepris-project}
v.project = '158416540'
f.scrape.gepris.project.desc(v.project)
