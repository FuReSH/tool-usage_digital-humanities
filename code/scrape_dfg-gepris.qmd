...
f.get.text <- function(html, xpath) {
  rvest::html_elements(html, xpath = xpath) %>%  # return all nodes for which the XPath is true
    rvest::html_text2()
}
```
...
f.scrape.gepris.project.desc <- function(project.id, delay) {
  ...
  #print(f.get.text(v.page, v.xpath.description))
  df.desc <- dplyr::tibble(
    project.id = as.character(project.id),
    # data from description
    # international cooperation: country
    collaborator.country = f.get.text(v.page, paste(v.xpath.description, 
            "div/span[@class = 'name'][contains(text(), 'Internationaler Bezug')]/following-sibling::span[@class = 'value']", sep = "/"))
  )
  df.details
}
```
...
v.project = '100000777' #'100000777' # '158416540'
...
f.scrape.gepris.project.collaborators <- function(project.id) {
  ...
  dplyr::tibble(
    ...
    #collaborator.country = f.get.text(v.page, paste(v.xpath.description, "div[@id='projektbeschreibung']//span[@class = 'name'][. = 'Internationaler Bezug']/follow-sibling::span[@class = 'value']", sep = "/")),
  )
}
```
...
We first create empty tibbles by running the scrape functions on an arbitrary project ID and then deleting all data from them.
...
for(project.id in df.ids.project[v.onset:v.terminus, ]$project.id) {
  print(paste0("Scraping DFG project no.", project.id, " from GEPRIS"))
  # test if the project ID is already present in the target tibble
  ifelse(df.projects$project.id == project.id,
         print(paste('Project no.', project.id, 'has already been scraped', sep = " ")),
